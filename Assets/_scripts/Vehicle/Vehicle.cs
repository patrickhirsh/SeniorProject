...
namespace Level
{
    ...
    public class Vehicle : MonoBehaviour
    {
        ...
        private void StartTraveling(Queue<Connection> connections)
        {
            ...
            _animationTween = StartCoroutine(TravelPath(vehicleCurve, connections.Peek()));
        }


        /// <summary>
        /// If the vehicle's currentConnection == null (can happen if TravelPath() is inturrupted by StopTraveling())
        /// then we need to "resolve" this vehicle's lost state. We do this on the next call to TravelPath() by 
        /// nesting this coroutine inside of it.
        /// </summary>
        private Coroutine resolveLostVehicle(Connection connection)
        {
            return StartCoroutine(TravelTo(connection));
        }
        ...
        {
            this.transform.LookAt(connection.transform);
            while(this.transform.position != connection.transform.position)
            {
                float step = RecoverySpeed * Time.deltaTime;
                Vector3.MoveTowards(this.transform.position, connection.transform.position, step);
                yield return null;
            }
